...
function addWorld(){
	...
	var positions = sphereGeometry.attributes.position;
	
	// Function to modify vertices
	function modifyVertex(index, modifier) {
		var x = positions.getX(index);
		var y = positions.getY(index);
		var z = positions.getZ(index);
	...
	for(var j = 1; j < tiers - 2; j++){
		for(var i = 0; i < sides; i++){
			var vertexIndex = (j * sides) + i;
			
			modifyVertex(vertexIndex, function(vertex) {
				var heightValue = (Math.random() * 0.07) - 0.035;
				var normalized = vertex.clone().normalize();
...
function createTree(){
	...
	// Convert to BufferGeometry and get position attribute
	var positions = treeGeometry.attributes.position;
	
	// Create a function to modify vertices
	function modifyVertex(index, modifier) {
		var x = positions.getX(index);
		var y = positions.getY(index);
		var z = positions.getZ(index);
	...
	// Modify vertices for the tree shape
	for(var i = 0; i < positions.count; i++) {
		modifyVertex(i, function(vertex) {
			var random = Math.random() * 0.1;
			vertex.x *= 1 + random;
			vertex.z *= 1 + random;
			vertex.y *= 1 + random * 0.5;
		});
	}
...
function update(){
    ...
    // Update lerp usage to use MathUtils
